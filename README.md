...
我分别实现了串行 的 v0 版本、 基于 CUDA 的 v1\~v8 版本、在 CPU 上构建 KD-Tree 并使用 OPENMP 并行化查询的 v9 版本、在 CPU 上构建 KD-Tree 拷贝到 GPU 上查询的 v10 版本、直接在 GPU 上构建 KD-Tree 并查询的 v11 版本。如下，如果要在它们之间进行切换，只需要修改 `sources/src/core.cu` 最后几行 `cudaCallback` 函数中实际调用的版本即可。
...
v5 版本中有一个 `cudaCallback` 函数作为外部调用的接口，还有一个 `cudaCallBackKernel` 核函数执行上文提到的求距离并执行规约过程。注意到使用的 v100 显卡计算能力为 7.0 ，对应的 [2d texture memory 中矩阵的宽度不能超过 65536]()，因此 v5 版本仅能处理 $n\le65536$ 大小的输入。如果超过这个限制，v5 版本会直接调用 v4 版本。
...
v11 版本直接在 GPU 上构建了一棵 KD-Tree 用于加速最近邻求解。
...
我一共实现了 v0\~v11 共 12 个版本。由于使用数据结构 KD-Tree 的 v9\~v11 版本比较特殊，有一个建树的过程，其 benchmark 方式和其他版本不同，因此对其性能的测试和评价在最后一部分进行（见下文）。
...
v3 版本相当于将 v2 版本中两个核函数融合，因此也继承了 v2 版本的缺点，在查询点只有 1 个的时候实际上只启动了 1 个 block。从运行时间上来看，对于 `{k, m, n} == {16, 1024, 65536}` 个时有一定的效果（11ms 减为 9ms），而在输入点只有 1 个的时候时间却增加了非常多（因为求距离时的并行性大大降低了）。
